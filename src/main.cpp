...
void setDecompositionTHREAD(const std::vector<Polygon_2>& osm_polys, const std::vector<Polygon_2>& atkis_polys, const std::vector<Polygon_2>& merged_polys,
                            std::pair<int,int> interval, const Localization& osm_rtree, const Localization& atkis_rtree,
    ...
    for (int p = interval.first; p <= interval.second; p++) {
        const Polygon_2 m_poly = merged_polys[p];
        ...
        std::vector<Polygon_2> osm_group, atkis_group;
        ...
        //write to Shapefiles
        std::string osm_set_path = folderMERGED + data_name + "_osm_set" + to_string(p);
        std::string atkis_set_path = folderMERGED + data_name + "_atkis_set" + to_string(p);
        writeToShapeFile(osm_group, osm_set_path);
        writeToShapeFile(atkis_group, atkis_set_path);
...
void solveConnectedSetTHREAD(std::string data_name, double lambda, int time_limit, int size_limit, std::pair<int,int> set_intervals,
    ...
    for(int set_id = set_intervals.first; set_id <= set_intervals.second; set_id++) {
        //measure time for anaylsis output
        std::chrono::steady_clock::time_point set_begin = std::chrono::steady_clock::now();

        //retreive paths
        std::string fileOSM = "../input/" + data_name + "/merged/" + data_name + "_osm_set" + to_string(set_id);
        std::string fileATKIS = "../input/" + data_name + "/merged/" + data_name + "_atkis_set" + to_string(set_id);

        //collect and output all matched unmatched polygons from osm and atkis polys
        std::vector <Polygon_2> osm_unconsidered, osm_considered;
        std::vector <Polygon_2> atkis_unconsidered, atkis_considered;

        //read OSM polygons from shapefile
        std::vector <Polygon_2> osm_polys, atkis_polys;
            ...
            for (int i = 0; i < poly_set.size(); i++) {
                for (int j = i + 1; j < poly_set.size(); j++) {
                    if (CGAL::do_intersect(poly_set[i], poly_set[j])) {
                        ...
                        if (inter_area > 0.1) {
                            double i_area = to_double(poly_set[i].area());
                            double j_area = to_double(poly_set[j].area());
                            if (i_area > j_area) {
        ...
        for (const auto &osm_p: osm_polys) {
            ...
            do {
                ...
                std::vector <Polygon_2> polys = map_switch ? osm_polys : atkis_polys;
                ...
                for (const auto &n_group: neighbors_grouped) {
                    ...
                    if (!completely_visited) {
                        ...
                        for (const auto &n: n_group) {
                            ...
                            std::vector <Polygon_2> polys_opp = !map_switch ? osm_polys : atkis_polys;
                            std::vector<int> neighbors;
...
void run_NtoM_with_PreDecomposition(const CommandLineOptions& options) {
    ...
    //read OSM polygons from shapefile
    std::vector <Polygon_2> osm_polys, atkis_polys, merged_polys;
    ReadShapeFile(fileOSM, &osm_polys);
    ReadShapeFile(fileATKIS, &atkis_polys);
    ...
    for(const auto& polyset: {osm_polys,atkis_polys,merged_polys}) {
        for(const auto& p : polyset)  {
            if(!p.is_simple()) {
                ...
                cout << "POLYGON((";
                for(const auto& v : p.vertices()) cout << std::fixed << v << ", ";
                cout<<"))\n";
            }
        }
    }
    ...
    for(int t=0; t < num_threads; t++) {
        decomp_threads[t] = std::thread(setDecompositionTHREAD,
                                     ...
                                     thread_set_intervals[t], std::ref(osm_rtree), std::ref(atkis_rtree),
    ...
    for(int t=0; t < num_threads; t++) {
        set_threads[t] = std::thread(solveConnectedSetTHREAD,
                                     data_name, lambda, time_limit, size_limit, thread_set_intervals[t],
